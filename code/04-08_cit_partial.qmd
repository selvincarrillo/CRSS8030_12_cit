...
Notes: Rain_cum was the most important variable with p-value < 0.05.CIT can handle numerical or categorical as predictors and as response variables. For highest yield, CIT says rain_cum, flag_leaf and first_n_rate.
...
Variables selected first (on top) are more important than variables selected afterwards. In this example, the most important variable in explaining grain yield is **Rain_Cum**.  Note: CIT is a multivariate model that does not suffer multicollinearity because it performs bivariate tests each time. 
...
Pros:  
  ...
  - It can model non-linear relationships  (and linear too)
